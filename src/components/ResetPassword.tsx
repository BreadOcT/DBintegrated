...
interface ResetPasswordProps {
  onNavigate: (page: 'landing' | 'login' | 'register' | 'forgot-password' | 'reset-password' | 'app') => void;
  token: string;
}
...
export function ResetPassword({ onNavigate, token }: ResetPasswordProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || 'Gagal mereset kata sandi. Tautan mungkin telah kedaluwarsa.');
      } else {
        setSuccess('Kata sandi berhasil direset! Silakan kembali ke halaman masuk.');
    ...
    <div className="min-h-screen bg-bg-base flex flex-col lg:flex-row font-sans relative overflow-hidden">
      ...
      <div className="w-full lg:w-[40%] flex flex-col px-6 py-6 sm:px-12 md:px-16 md:py-12 relative z-10 shadow-2xl bg-white/80 dark:bg-bg-base/80 backdrop-blur-2xl min-h-screen border-r border-sand mx-auto">
        ...
        >
            ...
            >
              Silakan masukkan kata sandi baru Anda untuk memulihkan akses ke akun Anda.
            </motion.p>
          ...
          {!success && (
            ...
            >
              <div className="space-y-2 group">
                <label className="text-xs font-bold text-text-main uppercase tracking-wider group-focus-within:text-nature-green transition-colors">Kata Sandi Baru</label>
                <div className="relative">
                  <input 
                    ...
                    className="w-full border border-sand bg-white dark:bg-bg-card rounded-xl pl-11 pr-12 py-3 text-sm font-semibold text-text-main focus:outline-none focus:border-nature-green focus:ring-1 focus:ring-nature-green transition-all placeholder:font-normal placeholder:text-sand hover:border-text-muted/50"
                  />
                  <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-text-muted group-focus-within:text-nature-green transition-colors" />
                  <button 
                    type="button"
              ...
              <div className="space-y-2 group">
                <label className="text-xs font-bold text-text-main uppercase tracking-wider group-focus-within:text-nature-green transition-colors">Konfirmasi Kata Sandi</label>
                <div className="relative">
                  <input 
              ...
              >
                {isLoading ? 'Menyimpan...' : 'Simpan Kata Sandi Baru'} <ArrowRight className="w-4 h-4 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all" />
              </button>
            </motion.form>
          )}
        </motion.div>
      ...
      <div className="hidden lg:flex w-[60%] bg-nature-green relative overflow-hidden flex-col items-center justify-center">
        ...
        <div className="text-center relative z-10 px-20">
            <h2 className="text-5xl font-bold leading-[1.1] text-bg-base mb-6">Mulai lembaran baru.</h2>
            <p className="text-bg-base/80 text-xl font-medium">Amankan akun Anda dengan kata sandi yang kuat dan mudah diingat.</p>
        </div>
      </div>
    </div>
  );
